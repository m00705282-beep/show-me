 ...
 * Automatski bira NAJBOLJU priliku sa ENABLED exchanges
 */
...
console.log(chalk.cyan.bold('║         🚀 SMART ARBITRAGE TRADE EXECUTOR 🚀             ║'));
console.log(chalk.cyan.bold('╚════════════════════════════════════════════════════════════╝\n'));
...
// Get opportunities that match configured exchanges
...
async function main() {
  try {
    console.log(chalk.yellow('📡 Fetching valid opportunities...\n'));
    
    const opps = await getValidOpportunities();
    
    if (opps.length === 0) {
      console.log(chalk.red('❌ No valid opportunities found with configured exchanges.'));
      console.log(chalk.yellow('\n💡 Make sure you have API keys for at least 2 exchanges.\n'));
      process.exit(0);
    }
    
    console.log(chalk.green(`✅ Found ${opps.length} valid opportunities!\n`));
